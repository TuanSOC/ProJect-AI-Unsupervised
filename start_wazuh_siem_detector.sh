...
# 
# ⚠️ CẢNH BÁO: Script này chỉ dùng để TEST/DEBUG
# 
...
if command -v systemctl &> /dev/null; then
    if systemctl is-active --quiet wazuh-siem-sqli-detector.service 2>/dev/null; then
        echo "⚠️  WARNING: Systemd service 'wazuh-siem-sqli-detector' is already running!"
...
if [ "$MODEL_FOUND" = false ]; then
    echo "⚠️  Warning: Model file không được tìm thấy!"
...
if [ ! -f "$LOG_PATH" ]; then
    echo "⚠️  Warning: Log file không tồn tại: $LOG_PATH"
...
echo ""
echo "⚠️  Lưu ý: Đây là chế độ test/debug."
